...
const FLOW_PAGES: readonly Workspace[] = ["import", "diagnosis", "interaction", "export"] as const;

const NAV_ITEMS: readonly { key: Workspace; label: string; group: string }[] = [
  { key: "overview", label: "总览", group: "工作区" },
  { key: "import", label: "财报导入", group: "工作区" },
  { key: "budget", label: "模板工作台", group: "工作区" },
...
function Topbar({
  ...
  const stepLabel = NAV_ITEMS.find((n) => n.key === current)?.label ?? "总览";
  const healthLabel =
    health.status === "ok"
      ? `后端 ${health.bind} 已连接`
...
function WorkflowRail({
  ...
  const doneMap: Record<string, boolean> = {
    import: Boolean(session.session),
  ...
  return (
    <div className="workflow-wrap">
      ...
      {session.session ? (
        <span className="workflow-wrap__note">
          {session.session.company_name} · {session.session.years.join(" / ")} 年
        </span>
      ) : (
        <span className="workflow-wrap__note">尚未导入数据</span>
      )}
    </div>
  );
}
...
function StepNav({
  ...
  const goNext = () => {
    if (!next) return;
    if (current === "import" && !session.session) {
      setBlockMsg("请先完成财报导入（拖入文件或载入示例数据）");
      return;
    }
  ...
  return (
    <>
      ...
      <section className="panel">
        ...
        <div className="ai-actions">
          <button type="button" className="btn btn--ai" disabled={aiBusy || aiBlocked || !session.session} onClick={onGenerateReport}>
            {aiBusy ? "AI 合并中…" : aiReport || aiJob?.status === "failed" ? "重新生成 AI 报告" : "生成 AI 合并报告"}
          </button>
          <span className={`ai-config__status${aiConfigured ? " is-on" : ""}`}>
            {aiConfigured ? "● AI 已配置" : "○ 未配置（可在设置中配置）"}
          </span>
        </div>
        {aiError ? <div className="status status--error" role="alert">{aiError}</div> : null}
        {aiJob && (aiJob.status === "queued" || aiJob.status === "running") ? (
          <div className="ai-progress" aria-live="polite">
            <div className="ai-progress__head">
              <strong>{aiJob.message || (aiJob.status === "queued" ? "报告任务排队中" : "正在生成跨年合并报告")}</strong>
              <span>
                {aiJob.progress.total > 0
                  ? `${aiJob.progress.current} / ${aiJob.progress.total}`
                  : `${aiJob.progress.current} / 总页数待确认`}
              </span>
            </div>
            <div
              className={`ai-progress__track${aiJob.progress.total > 0 ? "" : " is-indeterminate"}`}
              role="progressbar"
              ...
              {...(aiJob.progress.total > 0
                ? { "aria-valuemin": 0, "aria-valuemax": aiJob.progress.total, "aria-valuenow": aiJob.progress.current }
                : {})}
            >
              <span
                className="ai-progress__bar"
                style={{ width: aiJob.progress.total > 0 ? `${(aiJob.progress.current / aiJob.progress.total) * 100}%` : "35%" }}
              />
            </div>
          </div>
        ) : null}
      ...
      <section className="panel">
        ...
        {selectedReport ? (
          ...
          <p className="panel__note">暂无报告记录。生成 AI 报告后会自动保存在这里。</p>
        )}
      </section>
      <section className="panel">
        <h2 className="panel__title">流程进度</h2>
        <p className="panel__note">
          当前步骤进度见页面顶部步骤条；诊断、互动、导出将按流程逐环节接入。
        </p>
      </section>
    </>
  );
...
function ImportPage({
  ...
  const onLoadSample = async () => {
    ...
    setShowSavedPreview(false);
    try {
      const resp = await importSample();
      onImported(resp);
      setCompanyName(resp.summary.company_name);
      setIndustry(resp.summary.industry);
      setIdentifyInfo(null);
      setNotice(`示例数据已载入：${resp.summary.company_name}`);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
      setBusy(false);
    ...
    <>
      ...
      <section className="panel">
        ...
        <div className="action-row">
          ...
          <button type="button" className="btn btn--ghost" disabled={busy} onClick={() => void onLoadSample()}>
            载入示例数据
          </button>
          {session.session ? (
      ...
      {session.session ? (
        <section className="panel">
          ...
          {/* 导入后自动 AI 总结：逐页分段分析（每年独立 → 合并对比），完成后预览 */}
          <div className="ai-summary-block">
            <h3 className="ai-summary-title">AI 总结预览</h3>
            {aiJob && (aiJob.status === "queued" || aiJob.status === "running") ? (
              <div className="ai-progress" aria-live="polite">
                <div className="ai-progress__head">
                  <strong>{aiJob.message || (aiJob.status === "queued" ? "AI 总结排队中" : "正在逐页分析并总结")}</strong>
                  <span>
                    {aiJob.progress.total > 0
                      ? `${aiJob.progress.current} / ${aiJob.progress.total} 页`
                      : `${aiJob.progress.current} / 总页数待确认`}
                  </span>
                </div>
                <div
                ...
                <p className="ai-summary-hint">
                  每份文件单独分析 → 合并跨年对比，避免一次输入过多导致截断；完成后在此直接预览总结。
                </p>
              </div>
            ) : aiJob?.status === "failed" ? (
              <div className="status status--error" role="alert">
                AI 总结失败：{aiJob.error || "未知错误"}（可到「总览」页重新生成）
              </div>
            ) : aiReport ? (
              <div className="ai-result">
                <MarkdownBlock text={aiReport} />
              ...
              <p className="panel__note">AI 总结已生成，可到「总览」页查看完整报告。</p>
            ) : (
              <p className="panel__note">
...
function BudgetPage() {
  ...
  const [error, setError] = useState("");
  const [editable, setEditable] = useState<{ budget_revenue: string; budget_cost: string; last_year_revenue: string; last_year_cost: string } | null>(null);
  ...
  useEffect(() => {
    fetchBudgetFromSession()
      .then((resp) => {
        setPlan(resp);
        setEditable({
          budget_revenue: String(resp.plan.top_inputs.budget_revenue),
          budget_cost: String(resp.plan.top_inputs.budget_cost),
          last_year_revenue: String(resp.plan.top_inputs.last_year_revenue),
          last_year_cost: String(resp.plan.top_inputs.last_year_cost),
        });
      })
  ...
  return (
    <section className="panel">
      <h2 className="panel__title">模板工作台</h2>
      {error ? (
        <div className="status status--error">{error}</div>
      ) : !plan || !editable ? (
        <div className="placeholder">
          <strong>84 行预算模板</strong>
          <span>正在加载模板工作台…</span>
        </div>
      ) : (
        <>
          <div className={`budget-source budget-source--${plan.method}`}>
            <span className="budget-source__badge">
              {plan.method === "ai" ? "AI 识别" : "结构化提取"}
            </span>
            <span className="budget-source__note">{plan.source_note}</span>
          </div>
          <div className="budget-top">
            <h3 className="budget-top__title">顶部输入（TopInputs）</h3>
            <div className="budget-top__grid">
              {(
                [
                  ["budget_revenue", "预算营业收入（C2）", "元"],
                  ["budget_cost", "预算营业成本（C3）", "元"],
                  ["last_year_revenue", "上年度同期营业收入（C6）", "元"],
                  ["last_year_cost", "上年度同期营业成本（C8）", "元"],
                ] as const
              ).map(([key, label, unit]) => (
                <label key={key} className="field">
                  <span className="field__label">{label}（{unit}）</span>
                  <input
                    className="input"
                    type="number"
                    value={editable[key]}
                    onChange={(e) => setEditable((s) => (s ? { ...s, [key]: e.target.value } : s))}
                  />
                </label>
              ))}
            </div>
            <div className="budget-top__meta">
              <span>企业：<strong>{plan.plan.company_name}</strong></span>
              <span>行业：<strong>{plan.plan.industry}</strong></span>
              <span>预算年度：<strong>{plan.plan.year}</strong></span>
            </div>
          </div>
          <div className="placeholder placeholder--sm">
            <strong>84 行预算明细</strong>
            <span>明细编辑、筛选搜索与三类导出将在后续阶段接入</span>
            <span className="placeholder__hint">
              已从审计报告识别：营业收入 {fmt(plan.plan.top_inputs.budget_revenue)} 元（本年）
              · {fmt(plan.plan.top_inputs.last_year_revenue)} 元（上年）
            </span>
          </div>
        </>
      )}
      ...
      <section className="panel">
        ...
        {!session.session ? (
          <div className="diag-state">
            ...
            <p className="panel__note">尚未导入财报。请先前往「财报导入」上传文件或载入示例数据，再进行诊断。</p>
          </div>
        ) : busy ? (
