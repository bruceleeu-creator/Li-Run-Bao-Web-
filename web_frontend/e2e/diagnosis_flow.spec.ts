...
test("步骤条在导入页可见，未导入时下一步被拦截", async ({ page }) => {
  ...
  await page.goto("/");
  // 步骤条在内容顶部可见
  await expect(page.getByText("财报导入", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("第一轮诊断", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("A/B/C 互动", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("第二稿与导出", { exact: true }).first()).toBeVisible();
