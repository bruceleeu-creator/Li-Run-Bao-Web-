...
/** UI 导入路径夹具：通过真实上传控件触发 handleImported（示例按钮已移除） */
const importFixture = path.resolve(__dirname, "../../web_backend/workspaces/2021年审计报告.xlsx");

async function uploadViaUi(page: Page) {
  await page.setInputFiles('input[type="file"][accept]:not([webkitdirectory])', importFixture);
...
async function mockAppShell(page: Page, activeJob: Job | null = null) {
  ...
  await page.route("**/api/industries", (route) =>
    fulfillJson(route, { industries: [{ name: "制造业", desc: "" }], names: ["制造业"], default: "制造业" }),
  );
