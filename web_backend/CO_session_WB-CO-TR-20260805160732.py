...
def from_persisted_dict(raw: dict) -> FinancialData:
    """持久化 dict → FinancialData 的忠实还原（与 _to_dict 互逆）。

    与 parser.parse_financial_dict 的区别：后者面向解析器输出的样例 JSON，
    会丢弃 parsed_meta（policy/data_quality/reconciliation 等）并破坏
    account_balances 的年份维度——历史载入/重启恢复必须走本函数，
    否则版本指纹变化、政策与质量信息丢失。
    """
    def _year_map(value):
        # {年份(可能是字符串键): 金额} → {int 年: 金额}
    ...
    return FinancialData(
        ...
        years=years,
        income_statement={k: _year_map(v) for k, v in (raw.get("income_statement") or {}).items()},
        balance_sheet={k: _year_map(v) for k, v in (raw.get("balance_sheet") or {}).items()},
        account_balances={k: _year_map(v) for k, v in (raw.get("account_balances") or {}).items()},
        parsed_meta=dict(raw.get("parsed_meta") or {}),
    )
...
def get_numeric_audit() -> dict:
    """当前会话数字质检报告（core.numeric_audit 产出，parsed_meta 持久化）。"""
    with _lock:
        data = _data
    if data is None:
        return {}
    raw = (data.parsed_meta or {}).get("numeric_audit")
    return dict(raw) if isinstance(raw, dict) else {}
