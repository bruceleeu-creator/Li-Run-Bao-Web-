...
| 2026-08-20 | P8 文档提前 | owner 因额度考虑要求提前：AGENTS.md v27 + README 已规整并入库——AGENTS 新增 v1.4.0 执行日志（**续作锚点**：四提交清单/P6 待办步骤与 SSH 公钥/部署包位置/便携 PG 命令/P7 剧本位置/P8 剩余）、月度拆分与协同看板两个架构专节、7 条新护栏、路径 C Windows 变体（credential fill+urllib+三坑）、实现状态表与验收命令更新；README 重写交付物②与 3.6 四步向导、新增 3.8 协同看板小节（访问地址待 P6 回填）、环境要求更新（Win/Py3.13）、目录结构与路线图更新。**P8 剩余：P7 归档后删除 specs/ + 全量门禁 + 终推** |
