...
> 更新：2026-09-09 v32.1 | **测试债清偿 + 仓库卫生（全局体检整改）**：①`core/diagnostic.py diagnose()` 开头快照 `original_industry`，结果 `industry` 用快照值——此前 `stamp_findings_tax_rate`→`pipeline._resolve_industry`（pipeline.py:132 `data.industry = key`）就地归一把结果污染成「制造业」（行业未匹配仅回退基准、保留原行业名；下游基准查询全自带兜底、web 路径 industry_fallback 恒 False 零影响；该断言自基线 e890c0f 起从未通过，非 v32 回归；pipeline 就地 mutation 根治列为后续可选）；②`tests/test_order_independence.py` 按 v32 互动契约重写：题队列统一取 start_session 之后的 `sess.interactive_ids`（含 auto 项剔除），7 例中 5 例重写、防跳项 2 例改用队列；新增「决策集合覆盖全部互动题」断言防空泛通过（`test_mixed_options_deterministic` 原即空泛通过，一并修复）；③CI 脚本删 5 行 `--deselect`（diagnostic 1 + order_independence 4），CI 等价 388→**393 绿**全回门禁（剩 full_ai_report 整文件 + pdf_scan 1 例为真实 PDF 夹具依赖，设计内排除）；④仓库卫生：.gitignore 补 `.zcode/`/`.hallmark/`、删 `!demo_output/sample_finance.xlsx` 悬挂白名单（该文件 666aed2 已出库、测试 fixture 自愈不再入库）、`.hallmark/log.json` git rm --cached 出库、清目录迁移残留的陈旧 __pycache__（曾致 pytest 回溯显示旧路径）；⑤**已推送（2026-09-09）**：两笔提交（chore 仓库卫生 `cc87c51` + fix 测试债 `2b27621`）经路径 C REST 直推（owner 当次授权；Mimosa 拦截的 7 high 为 2026-08-19 已分诊既有项、与本次改动无关），CI 七 job 全绿（2.5 分钟）→ deploy target=app 自动重建 8082，线上冒烟通过（诊断 industry=制造业/fallback=False、首题=CONSULTING_HIGH、6 题+2 auto）；线上锚点已随后续 docs 提交回填至 `2b27621`
> 更新：2026-09-07 v32 | **诊断聚焦 + 出题规则优化（诊断不到点上 → 重点先问、逐选项锚定）**：背景=owner 反馈「诊断部分诊断不到点上，导致最终答案受到影响；需优化诊断提问系统规则与选择题提问规则」；四组改动——①**互动提问聚焦**（`core/compliance_policy.sort_findings_for_interaction` + `core/interactive.start_session`）：互动顺序从「低→中→高（=诊断页展示序）」改为**高→中→低、同级按预计节税金额降序**（大额先问）；**低价值发现自动处理**（severity=低 且 max_est_saving < max(1000 元, 最新年营收×0.05%)，`is_low_value`）不再逐题问，自动按「暂维持」生成 auto 决策进第二稿（`[低影响·自动暂维持]` 前缀），auto 决策**不参与落地性扣分**（全 A=100% 契约保持）；诊断页低→高展示排序**不动**；②**选择题逐选项锚定**（`ai_engine.generate_options`）：v31 的整组拼接校验改为**每个选项独立过 `_text_is_grounded`**，任一套话选项→整组回退规则（消除「仅 A 锚定、B/C 套话」）；`_QUESTIONING_METHODOLOGY` 补第 8 条「选项逐项自检」；`_parse_options_json` risk_level 过 `_normalize_risk`（"偏高/较低"不再漏扣分）；前端 A/B/C 文案修正为「A·积极落地 / B·分阶段推进 / C·暂维持·防风险」（原「C·激进路径」与后端 C=暂维持语义相反）；③**AI 诊断产率与数值校验**（`discover_findings`）：schema 中 options 改**可选**（省略走既有逐条补题路径，防 10 条×3 选项大 JSON `finish_reason=length` 整批回退 0 条）；新增 `account` 字段（须真实科目名否则置空）挂接 `Finding.account_key`；fact 提示词要求首句「科目名+年份+金额」原值复述；**数值校验**：金额型 current_value 越案例锚点（容差 0.5%）→置 0、|est_saving|>营收→选项分栏清零（防污染第二稿/报告金额汇总）；④**第二稿趋势泛化**：`Finding` 新增 `account_key`（规则发现 12 处已填：研发费用/业务招待费/咨询服务费/福利费/教育经费/广告宣传费/销售·管理·财务费用/应收账款/营业收入/净利润；AI 发现经 account 字段挂接），`_compute_trend` 优先按科目多年序列（余额表优先、利润表次之）生成真实环比同比，无挂接回退原 4 个硬编码特判→「无历史」兜底；web 契约：`/api/interaction/state` 附 `auto_deferred` 概要与 `findings_total`，`total`=互动题数；前端互动页提示条列自动处理项。测试：锚定 14→**18**（+混合锚定整组拒/risk 归一/数值守卫/options 省略补题）、互动 12→**15**（+题序高→低/低价值 auto/科目趋势）、web 诊断互动 16→**17**（+互动首题=最高危与契约键）；`test_action_pack_entertain`/`test_template_export` 两处「空草稿」样例税金及附加 360,000→36,000（原值触发低严重度 VAT_HIGH 被 v32 自动处理，不再零发现）；**本地 CI 等价跑法（Windows）：`PYTHON_BIN=.venv/Scripts/python.exe bash scripts/CO_ci_pytest_WB-CO-TR-20260824.sh`**（脚本默认找 .venv/bin/python 仅 macOS/Linux）；**已推送上线（2026-09-07）**：commit `2475d020` 经路径 C REST 直推（github.com 443 仍超时、api.github.com 通；Mimosa 拦截的 7 high 全为 2026-08-19 已分诊既有项，test_interactive:138 为行号偏移的死端口假密钥夹具；owner 当次授权），本地经 ghproxy.net 镜像 fetch+reset 对齐；CI 七 job 全绿（5 分钟）→ deploy target=app 自动重建 8082 镜像、health OK；线上冒烟确认：样例 8 条发现中 2 条低价值自动暂维持（互动 6 题）、state 含 auto_deferred、首题=高严重度 CONSULTING_HIGH（重点先问生效）、前端 bundle 含新 A/B/C 文案无旧「激进路径」
> 更新：2026-08-31 v31.1 | **文档规整（无产品代码变更）**：修复本文件头部 v26 条目行尾误粘的 v25~v22 摘要重复文本（详细版内容已在「经营分析报告链路」专节，删除副本零信息损失）；测试计数核对刷新——`tests/` 33→**35 文件**（v30 Key 安全测试 + v31 锚定测试入库后未同步）、e2e 维持 11 spec 37 用例；README 同步——5.2/5.3 补 Windows 使用指引、5.7 验收命令对齐本文件单一真源（补 make_sample、删冗余子集行）、目录树 tests/scripts 描述更正、「全量 469」笔误更正为 CI 等价 380；「版本与发布状态」补线上版本锚点行；顺手补登记 v31 锚定测试进 guardian `EXPECTED_MODULES`（消「意外模块」警告，v31 会话遗漏）。P7/P8 状态复核无变化：specs/ 保留（删除门=P7 归档物落地，任务 36），线上版本仍为 `666aed2` · 2026-08-28
> 更新：2026-08-28 v31 | **AI 出题锚定加固（法律咨询式问诊，题目必须贴合本案例）**：背景=owner 反馈 AI 提问大量万金油套话、与本案财报无关；调研开源法律咨询项目（DISC-LawLLM 三段论 arXiv:2309.11325 / Intelligent Legal Assistant 澄清问诊 arXiv:2502.07904 / ChatLaw 禁离题负面清单 / HanFei 真实数据非占位符 / 律所 intake 问卷一事一问）后双管齐下——①`core/ai_engine.py` 新增 `_QUESTIONING_METHODOLOGY` 七条出题纪律（事实锚定/一事一问/先事实后提问/后果可预期/缺信息就澄清/不重复问/通用题自检）内嵌进 discover_findings、generate_options、enrich_interaction_question 三处提示词，enrich 补上此前完全没用的 `data` 参数（题面改写现在带企业数据摘要上下文）；②确定性锚定守卫 `_text_is_grounded`（AI 只出题、引擎把关，与月度拆分「AI 只出形状」同哲学）：选项组合/发现 fact/改写题面必须命中本案例数字锚点（科目金额+万元/亿元换算+年份+指标值，舍入容差 0.5%）或真实科目名，不锚定→选项抛错回退规则引擎、AI 发现直接丢弃、题面保留原文；月度拆分 `generate_monthly_questions` 同步要求 title 点名费用行目录中的具体科目，有效题<4 整体回退规则题库；新增 `tests/CO_test_question_grounding_WB-CO-TR-20260828.py` 19 例全绿、CI 等价全量 380 通过零回归（order_independence 4 失败为既有遗留，stash 对照证实）；**已推送上线（2026-08-28）**：commit `666aed2` 经 gh-proxy 镜像推送（当时直连 github.com 443 超时），CI 七 job 全绿（含内联「上线 · 服务器更新」job，注意 deploy 不是独立 workflow run，查状态看 CI run 的 jobs）自动部署 8082，服务器 lirunbao 镜像 12:12 重建、health OK
...
- 线上版本：http://49.232.160.7:8082 与 GitHub main 同步——2026-09-09 起 = `2b27621`（v1.4.0 + v30 Key 内存化 + v31 AI 出题锚定加固 + v32 诊断聚焦与出题规则优化 + v32.1 测试债清偿与仓库卫生），CI 全绿后自动部署；看板 8081 同机部署
...
- **互动聚焦与逐选项锚定（2026-09-07 v32，勿回退）**：①互动提问顺序用 `compliance_policy.sort_findings_for_interaction`（高→中→低、同级金额降序）——**不要**改回诊断页的 `sort_findings_by_severity`（低→高仅用于诊断页分区展示，两序解耦是本意）；②低价值发现（`interactive.is_low_value`：severity=低 且 max_est_saving < max(1000 元, 营收×0.05%)）由 `start_session` 自动生成 auto 决策（选 C 暂维持），**auto 决策不参与 `_compute_feasibility` 扣分**（保持全 A=100% 契约）；`Session.interactive_ids` 是提问队列、`auto_decisions` 是自动处理记录，完成条件按 interactive_ids 数；③`generate_options` 守卫是**逐选项独立**校验（任一选项不锚定→整组回退），`_parse_options_json` 的 risk_level 必须过 `_normalize_risk`；④`discover_findings` 的 options 字段可省略（省略走逐条补题，防大 JSON 截断整批回退）；AI 的 account 字段须真实科目名否则置空；金额型 current_value 越锚置 0、|est_saving|>营收分栏清零；⑤`Finding.account_key` 挂科目驱动第二稿真实趋势（`_account_series` 余额表优先），新增规则检查时能定位科目的必须填 account_key；⑥前端 A/B/C 文案「A·积极落地 / B·分阶段推进 / C·暂维持·防风险」与后端提示词语义一致，勿改回「C·激进路径」；⑦`/api/interaction/state` 的 `total`=互动题数（非全部发现数）、附 `auto_deferred` 概要与 `findings_total`
...
- **已知遗留失败的 CI 处理**：全量 pytest 走 `scripts/CO_ci_pytest_WB-CO-TR-20260824.sh`（`--ignore` full_ai_report 整文件 + `--deselect` pdf_scan 真实 PDF 夹具例 + `-m "not real_pdf"`；2026-09-09 v32.1 已清偿 diagnostic/order_independence 5 节点并删对应排除行）；**修复对应问题后必须同步删脚本里的对应排除行**，让测试回到门禁；本地等价验收直接 `bash scripts/CO_ci_pytest_WB-CO-TR-20260824.sh`
...
| 单元/接口测试 | `tests/` 35 文件 | ✅ 含 e2e 11 spec 37 用例（Playwright）+ 板端 tests_board 20 用例 |
...
- 本机已知遗留测试失败（与功能无关）：`CO_test_full_ai_report` 9 错误+部分失败（缺真实艺康 PDF，需上级「测试文件」目录）；`test_pdf_scan_parse::test_real_pdf_fixtures_are_project_portable`（同缺真实 PDF 夹具，2026-08-20 确认）。（2026-09-09 v32.1 已修复回门禁：`test_industry_fallback_to_manufacturing` 行业回退保留原名 + `test_order_independence` ×4 按 v32 互动契约重写，CI 排除行已删）
